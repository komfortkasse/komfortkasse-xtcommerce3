...
class Komfortkasse {
	...
	public function init() {
		...
		if ($current != '' && $current != 'undefined' && $current != $hashed) {
			echo "Access Code already set! Shop " . $current . ", given (hash) " . $hashed;
			return;
		}
		...
		if (extension_loaded('openssl')) {
			...
			$decrypt = self::kkdecrypt($crypttest, 'openssl');
		...
		if (!$encryptionstring && extension_loaded('mcrypt')) {
			...
			$decrypt = self::kkdecrypt($crypttest, 'mcrypt');
			if ($decrypt == "Can you hear me?") {
		...
		if (!$encryptionstring) {
			...
			$decrypt = self::kkdecrypt($crypttest, 'base64');
			if ($decrypt == "Can you hear me?") {
	...
	public function update($refunds) {
